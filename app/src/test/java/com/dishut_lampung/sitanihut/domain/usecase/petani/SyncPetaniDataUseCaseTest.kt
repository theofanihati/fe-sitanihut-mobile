...
class SyncPetaniDataUseCaseTest {
    private val repository: PetaniRepository = mockk()
    private lateinit var useCase: SyncPetaniDataUseCase

    @Before
    fun setUp() {
        useCase = SyncPetaniDataUseCase(repository)
    }

    @Test
    fun `invoke should return Success when repository sync is successful`() = runTest {
        val expectedResult = Resource.Success(Unit)
        coEvery { repository.syncPetaniData() } returns expectedResult

        val result = useCase()
        assertEquals(expectedResult, result)
        coVerify(exactly = 1) { repository.syncPetaniData() }
    }
