...
import com.dishut_lampung.sitanihut.presentation.shared.components.card.UserCard
...
private fun UserListScreenPreview() {
    SitanihutTheme {
        val dummyUsers = listOf(
            UserDetail(id = "1", name = "Adi Pratama", email = "adi@gmail,com.com", role = "petani", gender = "pria"),
            UserDetail(id = "2", name = "Naratama Ganendra", email = "naratama@gmail.com", role = "petani", gender = "pria")
        )
...
fun UserListRoute(
    ...
    UserListScreen(
        ...
        onRefresh = { viewModel.onEvent(UserEvent.OnRefresh) },
    ...
    Box(
            ...
            Column(
                ...
                Row(
                    ...
                    CustomSearchTextField(
                        modifier = Modifier.weight(1f),
                        query = state.query,
                        onQueryChange = { onEvent(UserEvent.OnSearchQueryChange(it)) },
                ...
                LazyColumn(
                    ...
                    items(
                        ...
                        UserCard(
                            ...
                            onActionClick = { onEvent(UserEvent.OnMoreOptionClick(user.id)) }
                        )
                    }
                }
            }
        ...
        if (state.isBottomSheetVisible) {
            GenericActionBottomSheet(
                onDismiss = { onEvent(UserEvent.OnBottomSheetDismiss) },
                onDetailClick = {
                    onEvent(UserEvent.OnBottomSheetDismiss)
                    state.selectedUserId?.let { onNavigateToDetail(it) }
                },
                onEditClick = {
                    onEvent(UserEvent.OnBottomSheetDismiss)
                    state.selectedUserId?.let { onNavigateToEdit(it) }
                },
                onDeleteClick = {
                    if (state.isOnline && state.userRole != "penanggung jawab") {
                        onEvent(UserEvent.OnDeleteClick)
                    }
                },
        ...
        if (state.isDeleteDialogVisible) {
            CustomConfirmationDialog(
                ...
                supportingText = "Data pengguna yang dihapus tidak dapat dikembalikan.",
                confirmButtonText = "Hapus",
                dismissButtonText = "Batal",
                onConfirm = { onEvent(UserEvent.OnDeleteConfirm) },
                onDismiss = { onEvent(UserEvent.OnDismissDeleteDialog) },
        ...
        AnimatedMessage(
            ...
            onDismiss = { onEvent(UserEvent.OnDismissSuccessMessage) },
            modifier = Modifier
        ...
        AnimatedMessage(
            ...
            onDismiss = { onEvent(UserEvent.OnDismissError) },
            modifier = Modifier
                .align(Alignment.BottomCenter)
                .padding(top = 80.dp)
        )
